...
class Avril_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'avril-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'avril_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'avril_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		$avril_customizer_notify_show_recommended_actions = get_option( 'avril_customizer_notify_show' );
		foreach ( $avril_customizer_notify_recommended_actions as $key => $avril_lite_customizer_notify_recommended_action ) {
			if ( $avril_customizer_notify_show_recommended_actions[ $avril_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $avril_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$avril_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $avril_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $avril_customizer_notify_recommended_action['plugin_slug'] );
				$avril_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $avril_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$avril_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$avril_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$avril_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$avril_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$avril_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$avril_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$avril_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$avril_lite_customizer_notify_recommended_action['button_label'] = $avril_deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $avril_lite_customizer_notify_recommended_action;
		}// End foreach().

		$customize_plugins = array();

		$avril_lite_customizer_notify_show_recommended_plugins = get_option( 'avril_customizer_notify_show_recommended_plugins' );

		foreach ( $avril_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $avril_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $avril_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			switch ( $active['needs'] ) {
				...
				case 'deactivate':
					...
					$ti_customizer_notify_recommended_plugin['button_label'] = $avril_deactivate_button_label;
					break;
			}
		...
		$json['total_actions']       = count( $avril_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="avril-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="avril-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="avril-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="avril-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no avril-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="avril-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="avril-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no avril-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
